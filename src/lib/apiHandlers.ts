...
export async function importFromDirectus(
  ...
  options?: {
    ...
    forceUpdate?: boolean;
    excludeRelationalFields?: boolean;
  }
  ...
  try {
    ...
    // Check if collection is singleton and get fields schema
    let isSingleton = false;
    let relationalFields: string[] = [];
    
    try {
      const sourceClient = new DirectusClient(sourceUrl, sourceToken);
      const collectionsRes = await sourceClient.get("/collections");
      const collections = collectionsRes.data || [];
      const collectionMeta = collections.find((c: any) => c.collection === collectionName);
      isSingleton = collectionMeta?.meta?.singleton === true;
      
      logStep("collection_type_check", { 
        collectionName, 
        isSingleton,
        meta: collectionMeta?.meta 
      });

      // Get fields to identify relational fields if needed
      if (options?.excludeRelationalFields) {
        try {
          const fieldsRes = await sourceClient.get(`/fields/${collectionName}`);
          const fields = fieldsRes.data || [];
          
          relationalFields = fields
            .filter((f: any) => {
              const meta = f.meta;
              // Check if field is relational (foreign key, many-to-one, one-to-many, many-to-many)
              return meta?.special?.includes('m2o') || 
                     meta?.special?.includes('o2m') || 
                     meta?.special?.includes('m2m') ||
                     meta?.special?.includes('m2a') ||
                     f.type === 'alias';
            })
            .map((f: any) => f.field);
          
          logStep("relational_fields_detected", { 
            collectionName, 
            relationalFields,
            count: relationalFields.length 
          });
        } catch (fieldsErr: any) {
          logStep("fields_fetch_failed", { error: fieldsErr.message });
        }
      }
    } catch (err: any) {
      logStep("collection_type_check_failed", { error: err.message });
    }

    // Skip token validation - already validated during connection test
    logStep("token_validation_skipped", { 
      reason: "Already validated during initial connection"
    });
    ...
    logStep("fetch_data_start", { collectionName, isSingleton, titleFilter: options?.titleFilter });
    
    let sourceItems: any[] = [];
    
    if (isSingleton) {
      // For singleton: GET /items/{collection} (no limit, no ID)
      const sourceClient = new DirectusClient(sourceUrl, sourceToken);
      try {
        const response = await sourceClient.get(`/items/${collectionName}`);
        const singletonData = response.data || response;
        
        if (singletonData && typeof singletonData === 'object') {
          sourceItems = [singletonData];
          logStep("singleton_fetch_success", { collectionName, data: singletonData });
        } else {
          logStep("singleton_empty", { collectionName });
        }
      } catch (fetchErr: any) {
        logStep("singleton_fetch_failed", { 
          collectionName, 
          error: fetchErr.message,
          status: fetchErr.response?.status 
        });
        throw new Error(`Failed to fetch singleton: ${fetchErr.message}`);
      }
    } else {
      // For normal collections: use readItems with limit
      ...
      sourceItems = Array.isArray(response) ? response : [];
    ...
    // For singleton collections, only process the first item
    const itemsToImport = isSingleton 
    ...
    for (let i = 0; i < itemsToImport.length; i++) {
      ...
      try {
        ...
        // Exclude relational fields if requested
        if (options?.excludeRelationalFields && relationalFields.length > 0) {
          relationalFields.forEach(field => {
            delete itemToImport[field];
          });
          
          logStep("relational_fields_excluded", { 
            itemId: isSingleton ? collectionName : id,
            excludedFields: relationalFields 
          });
        }
        ...
        // Singleton collections: use /items/{collection} without ID
        if (isSingleton) {
          try {
            logStep("singleton_update_start", { collectionName });
            // For singleton, always use PATCH to update the single record
            importResponse = await targetClient.patch(
              `/items/${collectionName}`,
              itemToImport,
            );
            action = "updated";
            logStep("singleton_updated", { collectionName });
          } catch (updateErr: any) {
            logStep("singleton_update_failed", { 
              collectionName,
          ...
          // Normal collections: use /items/{collection}/{id}
        ...
        importedItems.push({
          originalId: isSingleton ? collectionName : id,
          newId: importResponse.data?.id || (isSingleton ? collectionName : id),
          status: "success",
          action,
        ...
        errorCount++;
        importedItems.push({
          originalId: isSingleton ? collectionName : item.id,
          status: "error",
          error: {
...
export async function importSelectedItems(
  ...
  try {
    ...
    if (isSingleton) {
      // For singleton, fetch directly without filtering by ID
      try {
        const sourceClient = new DirectusClient(sourceUrl, sourceToken);
        const response = await sourceClient.get(`/items/${collectionName}`);
        const singletonData = response.data || response;
        ...
        if (singletonData && typeof singletonData === 'object') {
          sourceItems.push(singletonData);
          logStep("singleton_fetch_success", { collectionName });
        }
      } catch (err: any) {
        logStep("singleton_fetch_failed", { collectionName, error: err.message });
      }
    } else {
      // For normal collections, fetch by IDs
