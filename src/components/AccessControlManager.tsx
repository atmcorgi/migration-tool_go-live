...
export function AccessControlManager({ 
  ...
  const [migrationOptions] = useState<AccessControlMigrationOptions>({
    roles: { preserveIds: true, skipAdminRoles: false },
    policies: { preserveIds: true, skipAdminPolicies: false },
  ...
  const getGroupedPermissions = () => {
    ...
    const largePolicyIds = Object.keys(policyPermissionCounts)
      ...
      .sort((a, b) => policyPermissionCounts[b] - policyPermissionCounts[a]); 
  ...
  const getPaginatedPermissions = (permissions: any[], groupKey: string) => {
    const isCollapsed = collapsedGroups[groupKey] !== undefined ? collapsedGroups[groupKey] : true; 
  ...
  const loadSourceData = async () => {
    ...
    try {
      ...
      const [sourceRolesRes, sourcePoliciesRes, sourcePermissionsRes, sourceAccessRes] = await Promise.all([
        ...
        sourceClient.get('/access', { params: { limit: -1 } }).catch(() => ({ data: [] }))
      ]);

      const [targetRolesRes, targetPoliciesRes, targetPermissionsRes] = await Promise.all([
        targetClient.get('/roles').catch(() => ({ data: [] })), 
        targetClient.get('/policies').catch(() => ({ data: [] })),
        targetClient.get('/permissions').catch(() => ({ data: [] })) 
      ]);
      ...
      setSourceRoles(sourceRolesWithPolicies); 
