...
interface SettingsManagerProps {
  ...
  onLoadSettings: (
    settings: Omit<ConnectionSettings, "name" | "createdAt">
  ) => void;
  onClearAll: () => void;
}
...
export function SettingsManager({
  ...
  const getSavedPresets = (): ConnectionSettings[] => {
    try {
      ...
      const parsed: ConnectionSettings[] = saved ? JSON.parse(saved) : [];

      // Sanitize: never expose tokens from stored presets
      ...
      // Persist sanitized version back to storage
      localStorage.setItem(
        "directus-migration-presets",
  ...
  const [savedPresets, setSavedPresets] = useState<ConnectionSettings[]>(
    getSavedPresets()
  );
  ...
  const saveCurrentAsPreset = () => {
    ...
    const newPreset: ConnectionSettings = {
      ...
      // Do not persist tokens in presets
      sourceToken: "",
  ...
  const loadPreset = (preset: ConnectionSettings) => {
    onLoadSettings({
      ...
      // Never load tokens from presets back into the form
      sourceToken: "",
  ...
  const clearAllData = () => {
    if (
      confirm(
        "This will clear all saved presets and current settings. Are you sure?"
      )
    ) {
  ...
  const exportSettings = () => {
    ...
    a.href = url;
    a.download = `directus-migration-settings-${
  ...
  return (
    ...
    >
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: "0.75rem", color: "#6b7280" }}>
                      {preset.sourceEnvironment} ({preset.sourceUrl}) →{" "}
                      {preset.targetEnvironment} ({preset.targetUrl})
                    </div>
      ...
      >
        <h4
          style={{ margin: "0 0 0.5rem 0", fontSize: "1rem", color: "#dc2626" }}
        >
          ⚠️ Danger Zone
        </h4>
        <button
