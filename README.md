...
5. (Optional) Click “Validate Token” to check the token, or “Test Collections” to quickly verify collection permissions.
6. (Optional) Enter a limit to cap the number of items imported per collection.
7. For each collection you want to import, click “Import from another Directus”.
8. Monitor the import progress and review the status/output. Any failed items will be summarized in the console.
9. (Optional) Use the "History" feature to quickly reuse previously entered domains and tokens, or the "Clear History" options to remove them.
